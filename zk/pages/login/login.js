// pages/default/detail/detail.js
Page({
  onLoad: function (options) {
    let newlist = JSON.parse(wx.getStorageSync('remmond'))
    let newList = newlist.data.filter(v => v.id == this.options.id)
    this.setData({
      newObj: newList
    })
    wx.setStorageSync('remmond', JSON.stringify({
      "result": 0,
      "success": true,
      "data": [{
